...
export function generatePoints(
  ...
  pointType: 'positivePoint' | 'negativePoint'
): (number | null)[] {
  return Array.from({ length: endYear - startYear + 1 }, (_, i) => {
    const year = startYear + i;
    return Array.from({ length: 12 }, (_, j) => {
  ...
  return Array.from({ length: endYear - startYear + 1 }, (_, i) => {
    ...
    return Array.from({ length: 12 }, (_, j) => {
      ...
      const period = resumes.filter((e) => {
        ...
        return (
          year <= newYear &&
          newYear <= e.endDate.getFullYear() &&
          (year < newYear || month >= e.startDate.getMonth()) &&
          (newYear < e.endDate.getFullYear() || newMonth <= e.endDate.getMonth())
        );
      });
      return period;
    });
...
export async function getMotivationData(): Promise<{
  ...
  const [resumes, experiences] = await Promise.all([getResumes(), getExperiences()]);
  ...
  const negativePoints = generatePoints(startYear, endYear, experiences, 'negativePoint').map((point) =>
